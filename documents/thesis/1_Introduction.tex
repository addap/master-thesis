...
These are expressed using predicates in the logic.
...
The name comes from the fact that this proposition is by definition the weakest precondition \(P\) that makes the Hoare triple \(\{P\}\,e\,\{v.\,Q\,v\}\) true and Hoare triples are
even defined this way in Iris:
\[
...
Therefore, deriving a weakest precondition for an expression \(e\) proves its specification in terms of the assumptions \(P\) and conclusion \(Q\).
It will also establish the safety of the expression due to a soundness lemma of the logic.

One other powerful feature of Iris are \emph{shared invariants} \(\knowInv{\mathcal{N}}{I}\), which represent knowledge that a resource will not change over time, so they are also persistent.
...
In return, the client will receive from the effect handler a value \(w\) and gets to assume \(Q\).
In total, this can be thought of as an analogue to a Hoare triple like \(\{P\}\, handler\; v\, \{w,\; Q\; w\}\), where we explicitly name the handler that will handle the effect.
