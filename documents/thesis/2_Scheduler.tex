...
In \hazel{}s' protocol syntax they are formalized in the following way, where the precondition of \esuspend{} is given the name \isreg{} to describe the behavior of the fiber-defined \ocamlin{register} function.
...
Other pieces of ghost state are \gsPInv{}, \gsIsPr{}, and \gsReady{} described in figure~\ref{fig:logical-state-simpl}.
\gsPInv{} tracks additional resources for all existing promises by using an authoritative map which contains for each promise:
...
\gsIsPr{} is a persistent resource that denotes that \(p\) exists as a promise in \gsPInv{}. The \(pn\) ghost name is globally unique and included in the resource algebra we use for the proofs.
...
\begin{figure}[ht]
  \begin{align*}
    \gsPInv{}  \triangleq\;           & \exists M.\; \ownGhost{pn}{\authfull{}\; M} \ast                                                             \\
...
\gsPInv{} and \gsIsQueue{} were both necessary as preconditions because they describe global state, so they had to be passed around.
