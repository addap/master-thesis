...
The inner \ocamlin{execute} function is called once on each fiber to evaluate it and handle any performed effects.
\subsubsection*{Value Case}
The only non-effect case of the \ocamlin{match} just runs the next fiber because there are two types of fibers and their return value is always \ocamlin{()}.
\begin{itemize}
  \item The main fiber is wrapped in a saves its return value in a reference and returns \ocamlin{()}.
  \item All other fibers are forked using \ocamlin{Fiber.fork_promise}, which wraps them in a closure that saves their return value in a promise and returns \ocamlin{()}.
\end{itemize}
...
Handling a \efork{} effect is simple because it carries a new \ocamlin{fiber} to be executed, so the handler recursively calls the \ocamlin{execute} function to execute it immediately.
...
The fiber defines this condition by constructing a \ocamlin{register} function which in turn receives a wake-up capability by the scheduler in form of the \ocamlin{waker} function.
The key point is that as long as the continuation \ocamlin{k} is not invoked, the fiber will not continue execution.
The \ocamlin{waker} function places \ocamlin{k} into the run queue so that the fiber can continue execution by a call to the scheduler's \ocamlin{next} function.
The \ocamlin{register} function is called by the scheduler right after the fiber suspends execution and is responsible for installing \ocamlin{waker} as a callback at a suitable place (or even call it directly).
...
Waking up must be possible across thread boundaries, which is why the \ocamlin{run_queue} in the scheduler is thread-safe queue.
...
let fork_promise (f : () -> 'a) : 'a Promise.t =
  ...
  p
  \end{minted}
  \caption{Implementation of \ocamlin{Fiber.fork_promise}}
...
In figure~\ref{fig:sched-impl-cqs} we show the public API of the CQS module.
The \ocamlin{CQS.register} function registers a given callback with the data structure while \ocamlin{CQS.signal_all} calls all registered callbacks.
The return value of \ocamlin{CQS.register} depends on whether the function detects a parallel execution of \ocamlin{CQS.signal_all}.
If a parallel execution is detected, \ocamlin{CQS.register} will directly call the callback without waiting for \ocamlin{CQS.signal_all} to do it and return \ocamlin{Called}\footnote{This is purely an optimization. It would be equally valid for functional correctness to register the \ocamlin{waker} function in the data structure and let \ocamlin{CQS.signal_all} call it later.}.
Otherwise, \ocamlin{CQS.register} returns a \ocamlin{Registered handle} value, where \ocamlin{handle} can be used to call the \ocamlin{CQS.try_cancel} function.
\ocamlin{CQS.try_cancel} attempts to cancel a registered callback and returns whether the cancellation was successful.
If it was successful, the previously registered callback will not be called when \ocamlin{CQS.signal_all} is executed.
The implementation and specification of the functions will be expanded upon in section~\ref{sec:cqs}, for now we just explain their usage in the context of \ocamlin{Promise.await}.
...
val signal_all : t -> ()
  \end{minted}
  \caption{Interface of the \ocamlin{CQS} module.}
  \label{fig:sched-impl-cqs}
\end{figure}

In the \ocamlin{Promise.await} function if the promise is not fulfilled initially the fiber should wait until that is the case, so it performs a \esuspend{} effect.
The \ocamlin{register} function passed to the effect will register the \ocamlin{waker} function using \ocamlin{CQS.register}.
When at some point the \ocamlin{CQS.signal_all} function is called -- this happens in \ocamlin{Fiber.fork_promise} -- all registered \ocamlin{waker}s will be called in turn.
...
\begin{enumerate}
  ...
  \item The \ocamlin{waker} function is registered with CQS.
...
But because CQS is lock-free and promises can be shared between different threads there are a number of possible interleavings that the \ocamlin{register} function must take care of as well.
...
Right after the state is read in line 21 of figure~\ref{fig:sched-impl-await} another thread might change the state to \ocamlin{Done} and go on to call \ocamlin{CQS.signal_all}.
If that happens there is another race between the \ocamlin{CQS.register} in line 9 and the \ocamlin{CQS.signal_all} in the other thread.
If \ocamlin{CQS.register} detects that there is a racing \ocamlin{CQS.signal_all} it will directly call the \ocamlin{waker}.
...
To avoid this, \ocamlin{register} must check the state of the promise again in line 13, and if it is fulfilled try to cancel the \ocamlin{waker} registration.
The cancel will fail if the \ocamlin{waker} function was already called.
If it succeeds the \ocamlin{register} function has the responsibility of calling \ocamlin{waker} itself, which is done in line 16.

The only \textbf{safety} concerns in the above implementation are \ocamlin{Fiber.fork_promise} expecting the promise to be unfulfilled after the fiber has finished execution, and \ocamlin{Promise.await} expecting the promise to be fulfilled in the last match.
...
Both \ocamlin{register} and \ocamlin{waker} must not perform effects.
...
As long as the promise is not fulfilled we own a broadcast\todo{Establish \textit{broadcast} as a name for \ocamlin{CQS}}, one copy of \gspwait{}, and a \gssignal{}.
...
Of course, the \ewpt{} itself also implies \textbf{safety} of running both the main fiber and the scheduler.\todo{Don't ignore the return value.}
...
\begin{mathpar}
  ...
  { \gsPInv{} \ast \gsIsPr{}\; p\; \Phi \ast \gsIsCQS{}\; cqs }
  { \ewp{make\_register\; p\; cqs}{\bot}{reg,\; \gsIsReg{}\; reg\; (\lambda\; v,\; \ulcorner v = () \urcorner \ast \gspdone{})}}
...
\begin{mathpar}
  \inferrule[Spec-CQSRegister]
  { \gsIsCQS{}\; cqs \ast \gsIsCb{}\; callback\; R }
  { \ewp{CQS.register\; cqs\; callback}{\bot}{r,\; \textit{isCQSRegisterResult}\; r\; callback\; R}}

  \and

  \inferrule[Spec-CQSTryCancel]
  { \textit{isCQSRegisterHandle}\; h\; callback\; R }
  { \ewp{CQS.try\_cancel\; h}{\bot}{b,\; if\; b\; then\; \gsIsCb{}\; callback\; R\; else\; \top}}
\end{mathpar}

The function \ocamlin{CQS.register} takes a \(callback\) that satisfies the \gsIsCb{} predicate to register it in the CQS data structure.
This predicate is structurally similar to \gsIsWaker{} and, in fact, in the proof of \textsc{Spec-MakeRegister} we instantiate the precondition \(R\) with \(\gspdone{}\) and pass as the \ocamlin{callback} a \ocamlin{waker} function, which has the precondition \((\lambda\; v,\; \ulcorner v = () \urcorner \ast \gspdone{})\) as described above.
The result of \ocamlin{CQS.register} is either a value \(Called\), which expresses that it called the callback directly, or a register handle, which can be used to call \ocamlin{CQS.try_cancel}.

\ocamlin{CQS.try_cancel} will attempt to cancel a previous registration identified by the given \(handle\).
...
The proof of \textsc{Spec-MakeRegister} is straightforward and follows from the specifications of \ocamlin{CQS.register} and \ocamlin{CQS.try_cancel}.
