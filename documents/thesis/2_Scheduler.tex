...
This is the most complicated looking function in our case study which is partly due to the \esuspend{} effect and also due to the use of \emph{broadcast} functions.
...
The \ocamlin{Broadcast.register} function registers a given callback with the data structure while \ocamlin{Broadcast.signal_all} calls all registered callbacks.
The return value of \ocamlin{Broadcast.register} depends on whether the function detects a parallel execution of \ocamlin{Broadcast.signal_all}.
If a parallel execution is detected, \ocamlin{Broadcast.register} will directly call the callback and return \ocamlin{Called}\footnote{This is purely an optimization. It would be equally valid for functional correctness to register the \ocamlin{waker} function in the data structure and let \ocamlin{Broadcast.signal_all} call it later.}.
Otherwise, \ocamlin{Broadcast.register} returns a \ocamlin{Registered handle} value, where \ocamlin{handle} can be used to call the \ocamlin{Broadcast.try_cancel} function.
\ocamlin{Broadcast.try_cancel} attempts to cancel a registered callback and returns whether the cancellation was successful.
...
The \ocamlin{register} function passed to the effect will register the \ocamlin{waker} function using \ocamlin{Broadcast.register}.
...
Right after the state is read in line 21 of figure~\ref{fig:sched-impl-await} another thread might change the state to \ocamlin{Done} and go on to call \ocamlin{Broadcast.signal_all}.
If that happens there is another race between the \ocamlin{Broadcast.register} in line 9 and the \ocamlin{Broadcast.signal_all} in the other thread.
If \ocamlin{Broadcast.register} detects that there is a racing \ocamlin{Broadcast.signal_all} it will directly call the \ocamlin{waker}.
Otherwise, the \ocamlin{waker} is registered but in fact the \ocamlin{Broadcast.signal_all} might have already finished before \ocamlin{Broadcast.register} even started.
...
The result of \ocamlin{Broadcast.register} is either a value \(Called\), which expresses that it called the callback directly, or a register handle, which can be used to call \ocamlin{Broadcast.try_cancel}.

\ocamlin{Broadcast.try_cancel} will attempt to cancel a previous registration identified by the given \(handle\).
...
The proof of \textsc{Spec-MakeRegister} is straightforward and follows from the specifications of \ocamlin{Broadcast.register} and \ocamlin{Broadcast.try_cancel}.
